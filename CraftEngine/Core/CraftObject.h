...
namespace Craft  // 해당 클래스의 존재이유와 상세사항은 커스텀 RTTI 게시글 참조
{
	// 단일 상속만 허용하지만, 이는 CraftObject에서 파생받는 부모가 하나 (인터페이스나 ASC나 이런건 ok)
	...
	public:
		...
		{
            return false; // 최상위 CraftObject까지 재귀로 왔다는건 이미 같은 타입이나 상속받은 타입 아니라는 것
		}
...
/* CraftObject 를 상속받는 모든 클래스는 virtual 가상함수들을 재정의 해야하는데
